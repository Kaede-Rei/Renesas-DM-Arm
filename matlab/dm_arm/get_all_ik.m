function main()
    ...
    % 测试逆解
    q_test = [0, 0, 0, 0, 0, 0];
    T_test = dm_arm.fkine(q_test).T;
    
    % 获取所有可能的解
    all_solutions = get_all_inverse_kinematics(T_test, 0.1, dm_arm);
    ...
    if ~isempty(all_solutions)
        ...
        for i = 1:size(all_solutions, 1)
            ...
            % 验证每组解
            T_verify = dm_arm.fkine(all_solutions(i, :)).T;
